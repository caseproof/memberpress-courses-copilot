...
class ConversationFlowHandler extends BaseService
{
    ...
    private function inferUserPreference(array $conversationHistory): string
    {
        $preferenceIndicators = [
            'autonomy_requests' => preg_match_all('/\b(let me|I want to|I prefer|skip|jump)\b/i', 
                implode(' ', array_column($conversationHistory, 'content'))),
            'guidance_requests' => preg_match_all('/\b(help|guide|suggest|recommend|what should)\b/i', 
                implode(' ', array_column($conversationHistory, 'content'))),
            'collaboration_cues' => preg_match_all('/\b(we|together|collaborate|work with)\b/i', 
                implode(' ', array_column($conversationHistory, 'content')))
        ];
    ...
    private function getLinearBranches(string $currentState, array $transitions): array
    {
        ...
        return $nextState ? [['state' => $nextState, 'type' => 'linear', 'description' => 'Continue to next step']] : [];
    }
    ...
    private function getAdaptiveBranches(string $currentState, array $transitions, array $context): array
    {
        ...
        foreach ($transitions as $transition) {
            $skipConditions = $this->getBranchSkipConditions(['state' => $transition], 
